  ...
  return (
    <div className={`min-h-screen p-8 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} transition-colors duration-300`}>
      ...
      >
        ...
        <motion.div className={`p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          <div className={`w-full h-[60vh] p-4 rounded border overflow-auto transition-all duration-300 ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-gray-50 text-gray-900'}`} style={{ fontSize: `${fontSize}px`, fontFamily: fontName, lineHeight: lineHeight, letterSpacing: `${letterSpacing}em` }}>
            {noteText}
          </div>
        </motion.div>
